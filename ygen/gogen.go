...
type goStructField struct {
	...
	Description string // Description is the field's description taken from the YANG module.
...
// generatedGoStruct is used to represent a Go structure to be handed to a template for output.
type generatedGoStruct struct {
	...
	Module           string           // Module is the name of the YANG module where the schema for the struct is defined.
	Namespace        string           // Namespace is the namespace of the YANG module where the schema for the struct is defined.
...
// generatedGoContainerMethod contains fields required for generating the methods
// that are associated with a container entry (nested struct) within a struct representing a YANG entity.
type generatedGoContainerMethod struct {
	ContainerName  string  // ContainerName is the name of the container for which the method is being generated within its parent struct.
	ContainerType  string  // ContainerType is the type (struct name) of the container element.
	Receiver       string  // Receiver is the name of the parent struct of the container, which is the receiver for the generated method.
}
...
type generatedGoKeyHelper struct {
	...
	Keys []*yangFieldMap
	// KeysYANGOrder is a slice of the keys of the list in the order as defined in the YANG schema.
	KeysYANGOrder []string
}
...
type {{ .StructName }} struct {
    ...
    parent ygot.ValidatedGoStructExtended
{{- range $idx, $field := .Fields }}
    /* {{ if $field.Description }}{{ indentNextLines $field.Description 7 }}{{ end }}
     */
...
func (t *{{ .StructName }}) GetParent() ygot.ValidatedGoStructExtended { return t.parent }

// GetSchemaPath returns the container path in the YANG schema and also the module name and namespace.
func (*{{ .StructName }}) GetSchemaPath() (schemaPath, module, namespace string) {
	return "{{ .YANGPath }}",
           "{{ .Module }}",
           "{{ .Namespace }}"
}
...
`
	...
	// goSetContainerTemplate is a template that generates a setter function
	// for a nested struct field. The function generated creates the field if it does not exist yet.
	goSetContainerTemplate = `
// Set{{ .ContainerName }} creates or updates the value of the {{ .ContainerName }} struct field.
func (t *{{ .Receiver }}) Set{{ .ContainerName }}() {{ .ContainerType }} {
	if t.{{ .ContainerName }} != nil {
		return t.{{ .ContainerName }}
	}
	t.{{ .ContainerName }} = &{{ stripAsteriskPrefix .ContainerType }}{ parent: t }
	return t.{{ .ContainerName }}
}
`
...
`
	...
	// goOrderedKeyListTemplate defines the template for a function that is generated for a YANG
	// list type with multiple keys. It returns a slice of keys as defined for the list
	// in the YANG schema (preserving the order).
	goOrderedKeyListTemplate = `
{{- if gt (len .KeysYANGOrder) 1 }}
// ΛOrderedListKeys returns the keys of the {{ .Receiver }} struct, which is a YANG list entry,
// in the order as defined in the YANG schema.
func (t *{{ .Receiver }}) ΛOrderedListKeys() []string {
	return []string{
		{{- range $key := .KeysYANGOrder }}
		"{{ $key }}",
		{{- end }}
	}
}
...
`
	...
	goTemplates = map[string]*template.Template{
		...
		"keyListHelper":       makeTemplate("keyListHelper", goOrderedKeyListTemplate),
		...
		"setContainer":        makeTemplate("setContainer", goSetContainerTemplate),
	...
	templateHelperFunctions = template.FuncMap{
		...
		"indentNextLines": func(s string, indent int) string {
			var b bytes.Buffer
			p := strings.Split(s, "\n")
...
func writeGoStruct(targetStruct *Directory, goStructElements map[string]*Directory, state *genState, compressPaths, generateJSONSchema bool, goOpts GoOpts) (GoStructCodeSnippet, []error) {
	...
	if !targetStruct.IsFakeRoot {
		structDef.Namespace = targetStruct.Entry.Namespace().NName()
		structDef.Module, err = targetStruct.Entry.InstantiatingModule()
		if err != nil {
	...
	// associatedContainerMethods is a slice of pointers to generatedGoContainerMethod structs
	// which describe methods that use the target struct as a receiver. These structs
	// represent the methods that are used as helpers such as those methods that allow
	// to create a nested structure inside its parent.
	var associatedContainerMethods []*generatedGoContainerMethod
	...
	for _, fName := range GetOrderedFieldNames(targetStruct) {
		...
		case field.IsContainer():
			...
			associatedContainerMethods = append(associatedContainerMethods, &generatedGoContainerMethod{
				ContainerName: fieldDef.Name,
				ContainerType: fieldDef.Type,
		...
		if field.Namespace().NName() != targetStruct.Entry.Namespace().NName() {
			tagBuf.WriteString(` namespace:"`)
			tagBuf.WriteString(field.Namespace().NName())
			tagBuf.WriteByte('"')
		}
	...
	for _, method := range associatedContainerMethods {
		if err := goTemplates["setContainer"].Execute(&methodBuf, method); err != nil {
			errs = append(errs, err)
		}
	}
...
func generateGetListKey(buf *bytes.Buffer, s *Directory, nameMap map[string]*yangFieldMap) error {
	...
	for _, k := range s.ListAttr.KeyElems {
		h.KeysYANGOrder = append(h.KeysYANGOrder, k.Name)
	}

	err := goTemplates["keyHelper"].Execute(buf, h)
	if err != nil {
		return err
	}
	return goTemplates["keyListHelper"].Execute(buf, h)
}
